...
All 27 BNM Test Cases - Side by Side:

ID → Natural Language → Technical Format
...
3 → BNM - Payment Agreement - Reject Term → BNM_PaymentAgreement_Reject_Term_Failed
4 → BNM - Payment Agreement - Break Agreement After Rejection → BNM_PaymentAgreement_Break_AfterRejection
5 → BNM - Acquisition - Create Direct Debit for AP304 → BNM_Acquisition_Create_DirectDebit_AP304
6 → BNM - Acquisition - Pay Direct Debit Successfully → BNM_Acquisition_Pay_DirectDebit_Success
7 → BNM - Acquisition - Reject Direct Debit → BNM_Acquisition_Reject_DirectDebit_Failed
8 → BNM - Acquisition - Retry Direct Debit After Rejection → BNM_Acquisition_Retry_DirectDebit_AfterRejection
9 → BNM - BA Invoice - Create Direct Debit Successfully → BNM_BAInvoice_Create_DirectDebit_Success
10 → BNM - BA Invoice - Pay Direct Debit Successfully → BNM_BAInvoice_Pay_DirectDebit_Success
11 → BNM - BA Invoice - Reject Direct Debit → BNM_BAInvoice_Reject_DirectDebit_Failed
12 → BNM - BA Invoice - Pay Manually Before Batch DD → BNM_BAInvoice_Pay_Manual_BeforeBatchDD
13 → BNM - BA Invoice - Pay with Standard Payment Before Batch DD → BNM_BAInvoice_Pay_Standard_BeforeBatchDD
14 → BNM - Patient - Create Direct Debit via Portal → BNM_Patient_Create_DirectDebit_Portal
15 → BNM - Patient - Pay Direct Debit Successfully → BNM_Patient_Pay_DirectDebit_Success
16 → BNM - Patient - Reject Direct Debit → BNM_Patient_Reject_DirectDebit_Failed
17 → BNM - Credit - Create Installment Direct Debit → BNM_Credit_Create_Installment_DirectDebit
18 → BNM - Credit - Pay Installment Successfully → BNM_Credit_Pay_Installment_Success
19 → BNM - Credit - Reject Installment → BNM_Credit_Reject_Installment_Failed
20 → BNM - Claim - Pay with Standard Payment → BNM_Claim_Pay_Standard_Payment
21 → BNM - Insurance Receivable - Pay with Standard Payment → BNM_IR_Pay_Standard_Payment
22 → BNM - Helper - Confirm Direct Debit Payment → BNM_Helper_Confirm_DirectDebit_Payment
23 → BNM - Helper - Reject Direct Debit Payment → BNM_Helper_Reject_DirectDebit_Payment
24 → BNM - Helper - Process Direct Debit with Wrong Amount → BNM_Helper_Process_DirectDebit_WrongAmount
25 → BNM - Patient Portal - Create Account without Invoice → BNM_Portal_Create_Account_WithoutInvoice
26 → BNM - Patient Portal - Create Account with Invoice → BNM_Portal_Create_Account_WithInvoice
27 → BNM - Patient Portal - Link Claim to Account → BNM_Portal_Link_Claim_Account
